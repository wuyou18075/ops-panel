...
func NewAgentRecord() *AgentRecord {
	return &AgentRecord{
		...
		Prefs:    AgentPreferences{Group: "默认分组", TrackTraffic: true, DailyReport: false, Interval: 2, TrafficQuota: 1000 * 1024 * 1024 * 1024},
...
func enrollAgent(req EnrollRequest, masterAddr string) (*AgentRecord, string, error) {
	...
	if req.Group != "" {
		rec.Prefs.Group = req.Group
	}
	rec.Prefs.EnableConsole = req.EnableConsole
...
func RenameGroup(oldName, newName string) error {
	...
	agentsMu.Lock()
	for _, a := range agents {
		if a.Prefs.Group == oldName {
			a.Prefs.Group = newName
		}
	}
	_ = saveAgents(agentsFile)
	agentsMu.Unlock()
...
func UpdateAgentMeta(agentID, name string, prefs AgentPreferences) error {
	...
	if prefs.Interval < minInterval || prefs.Interval > maxInterval {
		prefs.Interval = rec.Prefs.Interval
	}
	if prefs.CountryCode != "" {
		prefs.CountryCode = strings.ToUpper(prefs.CountryCode)
	}
	rec.Name = name
	rec.Prefs = prefs
	if err := saveAgents(agentsFile); err != nil {
